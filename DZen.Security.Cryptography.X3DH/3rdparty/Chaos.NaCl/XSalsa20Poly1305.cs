...
namespace Chaos.NaCl
{
    ...
    {
        internal static readonly int KeySizeInBytes = 32;
        internal static readonly int NonceSizeInBytes = 24;
        internal static readonly int MacSizeInBytes = 16;

        internal static byte[] Encrypt(byte[] message, byte[] key, byte[] nonce)
        {
            if (message == null)
        ...
        internal static void Encrypt(ArraySegment<byte> ciphertext, ArraySegment<byte> message, ArraySegment<byte> key, ArraySegment<byte> nonce)
        {
        ...
        internal static byte[]? TryDecrypt(byte[] ciphertext, byte[] key, byte[] nonce)
        {
            if (ciphertext == null)
        ...
        internal static bool TryDecrypt(ArraySegment<byte> message, ArraySegment<byte> ciphertext, ArraySegment<byte> key, ArraySegment<byte> nonce)
        {
            if (key.Count != KeySizeInBytes)
                throw new ArgumentException("key.Length != 32");
            if (nonce.Count != NonceSizeInBytes)
